...
import { defaultOptions } from './tooltip.options';

@Component({
  selector: 'o-tooltip',
  templateUrl: './tooltip.component.html',
  host: { class: 'tooltip' },
  styleUrls: ['./tooltip.component.scss', './tooltip.light.scss'],
})
export class TooltipComponent implements OnInit {
  ...
  setHostStyle(
    placement: string,
    disableAutoPlacement: boolean = false
  ): boolean {
    ...
    if (placement === 'bottom') {
      topStyle =
        this.elementPosition.top + scrollY + elementHeight + this.tooltipOffset;
    }

    if (placement === 'top' || placement === 'bottom') {
      leftStyle =
        this.elementPosition.left + elementWidth / 2 - tooltipWidth / 2;
    }
  ...
  setAnimationDuration() {
    if (
      Number(this.options['animationDuration']) !=
      this.options['animationDurationDefault']
    ) {
      this.hostStyleTransition =
        'opacity ' + this.options['animationDuration'] + 'ms';
    }
  }
